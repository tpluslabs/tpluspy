...
def _place(
    ...
    if order_type == "limit":
        ...
        response = asyncio.run(
            client.create_limit_order(quantity=quantity, price=price, side=side, asset_id=asset)
        )
    else:
        response = asyncio.run(
...
def _list(
    ...
    if asset_id:
        result = asyncio.run(
            client.get_user_orders_for_book(AssetIdentifier(asset_id), open_only=open_only or None)
        )
    else:
        if open_only:
    ...
    if output_format == "raw":
        echo_with_pager(
            [order.model_dump_json(indent=2) for order in result],
            no_pager=no_pager,
        )
        return

    render([order.model_dump() for order in result], output_format, no_pager=no_pager)
